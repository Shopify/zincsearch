## Test ES API with endpoint /_bulk

1. ### Test Bulk Document Index
    This unit test will index documents in bulk:
    1. `POST` call to the endpoint `/_bulk` with bulkData in body.
...
2. ### Test Bulk Document Delete
    ...
    1. `POST` call to the endpoint `/_bulk` with bulkDataWithDelete in body.
...
3. ### Test Bulk Document with error Input
   ...
   1. `POST` call to the endpoint `/_bulk` with `{"index": {}}` in body .
