## Test ES API with Endpoint /:target/_bulk

1. ### Test Create Document with no existing Index
    This unit test will create document with not existing Index:
    1. `POST` call to the endpoint `/notExistIndex/_bulk` with bulkData in body.
...
2. ### Test Create Document with existing Index
    ...
    4. `POST` call to the endpoint `/"+indexName+"/_bulk` with bulkData in body.
...
3. ### Test Bulk with error Input
   ...
   1. `POST` call to the endpoint `/"+indexName+"/_bulk` with `{"index": {}}` in body .
